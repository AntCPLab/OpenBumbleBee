...
class PrimeRingCastProtocol {
 public:
  static constexpr size_t kHeuristicBound = 2;

  struct Meta {
    NdArrayRef prime;     // prime p
    int64_t prime_width;  // ceil(log2(p))
    FieldType dst_ring;   // target ring 2^k > p
    int64_t dst_width;    // ring width k
    // To do (approximated) truncation if specified.
    std::optional<int> truncate_nbits;
  };

  explicit PrimeRingCastProtocol(
      const std::shared_ptr<cheetah::BasicOTProtocols> &base);

  ~PrimeRingCastProtocol() = default;

  NdArrayRef Compute(const NdArrayRef &inp, const Meta &meta);

  // Given h0 + h1 = h mod p, compute z0 + z1 = h/2^d mod p
  NdArrayRef PrTruncateOverPrimeField(const NdArrayRef &inp, const Meta &meta);
 ...
 private:
  ...
  std::shared_ptr<cheetah::BasicOTProtocols> basic_ot_prot_ = nullptr;
};
