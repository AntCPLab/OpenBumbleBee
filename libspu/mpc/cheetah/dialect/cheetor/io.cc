...
#include "libspu/mpc/cheetah/dialect/cheetor/io.h"
...
Type CheetorIo::getShareType(Visibility vis, int owner_rank) const {
  if (vis == VIS_PUBLIC) {
...
std::vector<NdArrayRef> CheetorIo::toShares(const NdArrayRef& raw,
                                            Visibility vis,
...
template <typename T>
void ModuloPrime(NdArrayView<T> out, T prime) {
  seal::Modulus _prime(static_cast<uint64_t>(prime));

  pforeach(0, out.numel(), [&](int64_t i) {
    out[i] =
        seal::util::barrett_reduce_64(static_cast<uint64_t>(out[i]), _prime);
  });
}

template <>
void ModuloPrime(NdArrayView<uint128_t> out, uint128_t prime) {
  // FIXME(lwj): faster way for u128 prime modulo
  pforeach(0, out.numel(), [&](int64_t i) { out[i] %= prime; });
}

NdArrayRef CheetorIo::fromShares(const std::vector<NdArrayRef>& shares) const {
  ...
  if (eltype.isa<Public>()) {
    ...
    if (eltype.isa<PrimeShrTy>()) {
      // reconstruction is defined modulo prime
      auto ft = eltype.as<PrimeShrTy>()->field();
      DISPATCH_ALL_FIELDS(ft, "mod_prime", [&]() {
        ring2k_t prime = CheetorMulProt::GetPrimesProduct(ft).at<ring2k_t>(0);
        NdArrayView<ring2k_t> out(res);
        ModuloPrime(out, prime);
      });
    }
    return res;
  }
...
std::unique_ptr<CheetorIo> makeCheetorIo(FieldType field, size_t npc) {
  cheetor::registerTypes();

  return std::make_unique<CheetorIo>(field, npc);
}
