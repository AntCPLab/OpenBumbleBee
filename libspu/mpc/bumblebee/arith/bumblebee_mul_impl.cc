...
#include "libspu/mpc/bumblebee/rlwe/modswitch_helper.h"
#include "libspu/mpc/bumblebee/rlwe/utils.h"
...
struct BumblebeeMulImpl::Impl : public EnableCPRNG {
 public:
  ...
  std::vector<RLWECt> EncryptArray(const NdArrayRef &array);
...
void BumblebeeMulImpl::Impl::LocalExpandSEALContexts(size_t target) {
...
void BumblebeeMulImpl::Impl::LazyExpandSEALContexts(const Options &options,
  ...
  SPDLOG_INFO("BumblebeeMul uses {} modulus for {} bit input over {} bit ring",
              num_seal_ctx, options.msg_bitlen, options.ring_bitlen);
}

NdArrayRef BumblebeeMulImpl::Impl::MulOLE(const NdArrayRef &shr,
                                          yacl::link::Context *conn,
                                          bool evaluator,
  ...
  if (evaluator) {
    ...
    SPU_ENFORCE(encoded_mask.size() == payload_sze,
                "BeaverBumblebee: random mask poly size mismatch");
    // wait for IO
    io_task.get();
...
std::vector<RLWECt> BumblebeeMulImpl::Impl::EncryptArray(
  ...
  int64_t num_elts = inp.numel();
  int64_t num_splits = CeilDiv(num_elts, num_slots());
  int64_t num_seal_ctx = WorkingContextSize(options);
  int64_t num_polys = num_seal_ctx * num_splits;

  std::vector<RLWEPt> encoded_array(num_polys);
  EncodeArray(inp, /*scale_up*/ true, options, absl::MakeSpan(encoded_array));

  std::vector<RLWECt> encrypted_array(num_polys);
  ...
  yacl::parallel_for(0, num_polys, [&](int64_t job_bgn, int64_t job_end) {
    for (int64_t job_id = job_bgn; job_id < job_end; ++job_id) {
      ...
      // NOTE(lwj): symmetric encryption with seed
      encrypted_array[job_id] =
          sym_encryptors_[cntxt_id]
              ->encrypt_symmetric(encoded_array.at(job_id))
              .obj();
    }
  });
...
NdArrayRef BumblebeeMulImpl::Impl::MultiplyThenMask(
    const NdArrayRef &inp, absl::Span<RLWECt> recv_ct) {
  SPU_ENFORCE(inp.shape().size() == 1, "need 1D Array");
  SPU_ENFORCE(inp.numel() > 0);
  auto eltype = inp.eltype();
  SPU_ENFORCE(eltype.isa<Ring2k>(), "must be ring_type, got={}", eltype);
  ...
  int64_t num_seal_ctx = WorkingContextSize(options);
  int64_t num_polys = num_seal_ctx * num_splits;
  SPU_ENFORCE_EQ((size_t)num_polys, recv_ct.size());

  LazyExpandSEALContexts(options);
  LazyInitModSwitchHelper(options);

  std::vector<RLWEPt> encoded_array(num_polys);
  EncodeArray(inp, /*scale_up*/ false, options, absl::MakeSpan(encoded_array));

  std::vector<RLWEPt> encoded_mask;
  auto random_mask = PrepareRandomMask(field, num_elts, options, &encoded_mask);
  SPU_ENFORCE_EQ(encoded_mask.size(), recv_ct.size(),
                 "random mask poly size mismatch");

  yacl::parallel_for(0, recv_ct.size(), [&](int64_t job_bgn, int64_t job_end) {
    std::vector<uint64_t> u64tmp(num_slots(), 0);
    for (int64_t job_id = job_bgn; job_id < job_end; ++job_id) {
      int64_t cntxt_id = job_id / num_splits;
      auto &ct = recv_ct[job_id];
      ...
      // Multiply step
      CATCH_SEAL_ERROR(
          evaluator.multiply_plain_inplace(ct, encoded_array[job_id]));
      // H2A
      CATCH_SEAL_ERROR(evaluator.sub_plain_inplace(ct, encoded_mask[job_id]));
...
size_t BumblebeeMulImpl::Impl::EncryptArrayThenSend(const NdArrayRef &array,
                                                    const Options &options,
                                                    yacl::link::Context *conn) {
  int64_t num_elts = array.numel();
  auto eltype = array.eltype();
  ...
  for (int64_t i = 0; i < num_polys; i += kCtAsyncParallel) {
    ...
    conn->Send(
        nxt_rank, payload[i],
        fmt::format("BumblebeeMul::Send ct[{}] to rank={}", i, nxt_rank));
    for (int64_t j = 1; j < this_batch; ++j) {
      conn->SendAsync(
          nxt_rank, payload[i + j],
          fmt::format("BumblebeeMul::Send ct[{}] to rank={}", i + j, nxt_rank));
    }
  }
  return payload.size();
}

NdArrayRef BumblebeeMulImpl::Impl::PrepareRandomMask(
  ...
  // Ref: the one-bit approximate re-sharing in Bumblebee's paper
  // (eprint ver).
...
void BumblebeeMulImpl::Impl::EncodeArray(const NdArrayRef &array,
                                         bool need_encrypt,
                                         const Options &options,
...
void BumblebeeMulImpl::Impl::MulThenResponse(
    FieldType, int64_t num_elts, const Options &options,
    absl::Span<const yacl::Buffer> ciphers, absl::Span<const RLWEPt> plains,
    absl::Span<const RLWEPt> ecd_random, yacl::link::Context *conn) {
  SPU_ENFORCE(!ciphers.empty(), "BumblebeeMul: empty cipher");
  SPU_ENFORCE(plains.size() == ciphers.size(),
              "BumblebeeMul: ct/pt size mismatch");
  ...
  SPU_ENFORCE(ciphers.size() == (size_t)num_ciphers,
              "BumblebeeMul : expect {} != {}", num_ciphers, ciphers.size());
  SPU_ENFORCE(ecd_random.size() == (size_t)num_ciphers,
              "BumblebeeMul: encoded rnaomd size mismatch");
...
NdArrayRef BumblebeeMulImpl::Impl::DecryptArray(
    FieldType field, int64_t size, const Options &options,
    const std::vector<yacl::Buffer> &ct_array) {
  const int64_t num_splits = CeilDiv(size, num_slots());
  const int64_t num_seal_ctx = WorkingContextSize(options);
  ...
  SPU_ENFORCE(ct_array.size() == (size_t)num_ciphers,
              "BumblebeeMul: cipher size mismatch");
  SPU_ENFORCE(ms_helpers_.count(options) > 0);
...
void BumblebeeMulImpl::Impl::NoiseFloodCiphertext(
  ...
  for (size_t l = 0; l < num_modulus; ++l) {
    ...
    if (modulus[l].bit_count() > kNoiseFloodRandomBits) {
      // When prime[l] > 2^{kNoiseFloodRandomBits} then we add
      // directly add the random
      add_poly_coeffmod(&xrandom[0], dst_ptr, num_coeffs, modulus[l], dst_ptr);
    } else {
      // When prime[l] < 2^{kNoiseFloodRandomBits} we need to
      // compute modulo first.
...
std::vector<RLWECt> BumblebeeMulImpl::Impl::RecvEncryptedArray(
    FieldType field, int64_t numel, yacl::link::Context *conn) {
  SPU_ENFORCE(numel > 0 && field != FieldType::FT_INVALID);
  if (conn == nullptr) {
    conn = lctx_.get();
  }
  ...
  const int64_t num_splits = CeilDiv(numel, num_slots());
  const int64_t num_seal_ctx = WorkingContextSize(options);
  const int64_t num_ciphers = num_seal_ctx * num_splits;
  const int nxt = conn->NextRank();

  std::vector<RLWECt> out(num_ciphers);
  for (int64_t i = 0; i < num_ciphers; i += num_seal_ctx) {
    for (int64_t j = 0; j < num_seal_ctx; ++j) {
      auto buff = conn->Recv(nxt, "recv_ct");
      DecodeSEALObject(buff, seal_cntxts_.at(j), &out[i + j]);
    }
  }
  return out;
}

NdArrayRef BumblebeeMulImpl::Impl::RecvMulArray(FieldType field, int64_t numel,
                                                yacl::link::Context *conn) {
  SPU_ENFORCE(numel > 0 && field != FieldType::FT_INVALID);
  if (conn == nullptr) {
    conn = lctx_.get();
  ...
  std::vector<yacl::Buffer> buffer(num_ciphers);
  for (int64_t i = 0; i < num_ciphers; i++) {
    buffer[i] = conn->Recv(nxt, "recv_result");
  }
  return DecryptArray(field, numel, options, buffer);
}

void BumblebeeMulImpl::Impl::RandomizeCipherForDecryption(RLWECt &ct,
                                                          size_t context_id) {
...
BumblebeeMulImpl::BumblebeeMulImpl(std::shared_ptr<yacl::link::Context> lctx,
                                   bool allow_high_prob_one_bit_error) {
  impl_ = std::make_unique<Impl>(lctx, allow_high_prob_one_bit_error);
}

BumblebeeMulImpl::~BumblebeeMulImpl() = default;

int BumblebeeMulImpl::Rank() const { return impl_->Rank(); }

size_t BumblebeeMulImpl::OLEBatchSize() const {
  SPU_ENFORCE(impl_ != nullptr);
  return impl_->OLEBatchSize();
}

NdArrayRef BumblebeeMulImpl::MulOLE(const NdArrayRef &inp,
                                    yacl::link::Context *conn,
                                    bool is_evaluator,
                                    uint32_t msg_width_hint) {
  SPU_ENFORCE(impl_ != nullptr);
...
std::vector<RLWECt> BumblebeeMulImpl::RecvEncryptedArray(
    FieldType field, int64_t numel, yacl::link::Context *conn) {
  SPU_ENFORCE(impl_ != nullptr);
  return impl_->RecvEncryptedArray(field, numel, conn);
}

NdArrayRef BumblebeeMulImpl::RecvMulArray(FieldType field, int64_t numel,
                                          yacl::link::Context *conn) {
  SPU_ENFORCE(impl_ != nullptr);
  return impl_->RecvMulArray(field, numel, conn);
}

std::vector<RLWECt> BumblebeeMulImpl::EncryptArray(const NdArrayRef &inp) {
  SPU_ENFORCE(impl_ != nullptr);
  return impl_->EncryptArray(inp);
}

NdArrayRef BumblebeeMulImpl::MultiplyThenMask(const NdArrayRef &inp,
                                              absl::Span<RLWECt> recv_ct) {
  SPU_ENFORCE(impl_ != nullptr);
