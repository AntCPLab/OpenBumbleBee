...
class BumblebeeAlgoTest : public ::testing::Test {};

TEST_F(BumblebeeAlgoTest, Power4Int) {
  ...
  spu::mpc::utils::simulate(
      2, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
        ...
        std::shared_ptr<SPUContext> obj = makeBumblebeeProtocol(conf, lctx);
...
TEST_F(BumblebeeAlgoTest, Power4Fxp) {
  FieldType field = FM64;
  int fxp = 18;
  int64_t n = 81920;
...
TEST_F(BumblebeeAlgoTest, BatchLessThan) {
  FieldType field = FM64;
  int fxp = 18;
  int64_t n = 1 << 18;
...
TEST_F(BumblebeeAlgoTest, RingChange) {
  FieldType field = FM64;
