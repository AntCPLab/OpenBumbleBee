...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Combine(testing::Values(FieldType::FM64, FieldType::FM128),
                     testing::Values(
                         ...
                         //  Shape3D{18, 768, 78}
                         Shape3D{1, 40960, 256})),
    [](const testing::TestParamInfo<BumblebeeDotTest::ParamType>& p) {
...
TEST_P(BumblebeeDotTest, Basic) {
  ...
  for (int64_t stride : {1, 3}) {
    ...
    utils::simulate(kWorldSize, [&](std::shared_ptr<yacl::link::Context> lctx) {
      int rank = lctx->Rank();
      auto dot = std::make_shared<BumblebeeDot>(lctx);
...
TEST_P(BumblebeeDotTest, BatchDot) {
  size_t kWorldSize = 2;
  auto field = std::get<0>(GetParam());
  auto dim3 = std::get<1>(GetParam());
  int64_t B = 12;
