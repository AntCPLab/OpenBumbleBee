...
Type BumblebeeIo::getShareType(Visibility vis, int owner_rank) const {
  if (vis == VIS_PUBLIC) {
...
std::vector<NdArrayRef> BumblebeeIo::toShares(const NdArrayRef& raw,
                                              Visibility vis,
...
NdArrayRef BumblebeeIo::fromShares(
...
std::unique_ptr<BumblebeeIo> makeBumblebeeIo(FieldType field, size_t npc) {
  bumblebee::registerTypes();

  return std::make_unique<BumblebeeIo>(field, npc);
}
