...
class MITCCRHExp {
 public:
  ...
  emp::block keys[BatchSize];

  /**
  Renew with n explicitly specified AES keys.
  */
  void renew_ks(emp::block* new_keys, int n) {
  ...
  */
  void hash_exp(emp::block* out, const emp::block* in, int n) {
    int n_blks = (1 << n) - 1;
  ...
  */
  void hash_single(emp::block* out, const emp::block* in, int n) {
    int n_blks = n;
    for (int i = 0; i < n_blks; ++i) {
      out[i] = in[i];
    }
