...
struct CheetahMulImpl::Impl : public EnableCPRNG {
 public:
...
void CheetahMulImpl::Impl::LazyInitModSwitchHelper(const Options &options) {
...
void CheetahMulImpl::Impl::LocalExpandSEALContexts(size_t target) {
...
void CheetahMulImpl::Impl::LazyExpandSEALContexts(const Options &options,
                                                  yacl::link::Context *conn) {
  uint32_t target_plain_bitlen = TotalCRTBitLen(options);
  std::unique_lock<std::shared_mutex> guard(context_lock_);
...
NdArrayRef CheetahMulImpl::Impl::MulOLE(const NdArrayRef &shr,
                                        yacl::link::Context *conn,
                                        bool evaluator,
...
std::vector<RLWECt> CheetahMulImpl::Impl::EncryptArray(const NdArrayRef &inp) {
...
NdArrayRef CheetahMulImpl::Impl::MultiplyThenMask(const NdArrayRef &inp,
                                                  absl::Span<RLWECt> recv_ct) {
  SPU_ENFORCE(inp.shape().size() == 1, "need 1D Array");
  SPU_ENFORCE(inp.numel() > 0);
  auto eltype = inp.eltype();
...
size_t CheetahMulImpl::Impl::EncryptArrayThenSend(const NdArrayRef &array,
                                                  const Options &options,
                                                  yacl::link::Context *conn) {
  int64_t num_elts = array.numel();
  auto eltype = array.eltype();
...
NdArrayRef CheetahMulImpl::Impl::PrepareRandomMask(
  ...
  // Ref: the one-bit approximate re-sharing in Cheetah's paper
  // (eprint ver).
...
void CheetahMulImpl::Impl::EncodeArray(const NdArrayRef &array,
                                       bool need_encrypt,
                                       const Options &options,
...
void CheetahMulImpl::Impl::MulThenResponse(
...
NdArrayRef CheetahMulImpl::Impl::DecryptArray(
    FieldType field, int64_t size, const Options &options,
    const std::vector<yacl::Buffer> &ct_array) {
  const int64_t num_splits = CeilDiv(size, num_slots());
  const int64_t num_seal_ctx = WorkingContextSize(options);
...
void CheetahMulImpl::Impl::NoiseFloodCiphertext(
...
std::vector<RLWECt> CheetahMulImpl::Impl::RecvEncryptedArray(
...
NdArrayRef CheetahMulImpl::Impl::RecvMulArray(FieldType field, int64_t numel,
                                              yacl::link::Context *conn) {
  SPU_ENFORCE(numel > 0 && field != FieldType::FT_INVALID);
  if (conn == nullptr) {
    conn = lctx_.get();
...
void CheetahMulImpl::Impl::RandomizeCipherForDecryption(RLWECt &ct,
                                                        size_t context_id) {
...
CheetahMulImpl::CheetahMulImpl(std::shared_ptr<yacl::link::Context> lctx,
                               bool allow_high_prob_one_bit_error) {
  impl_ = std::make_unique<Impl>(lctx, allow_high_prob_one_bit_error);
}

CheetahMulImpl::~CheetahMulImpl() = default;

int CheetahMulImpl::Rank() const { return impl_->Rank(); }

size_t CheetahMulImpl::OLEBatchSize() const {
  SPU_ENFORCE(impl_ != nullptr);
  return impl_->OLEBatchSize();
}

NdArrayRef CheetahMulImpl::MulOLE(const NdArrayRef &inp,
...
std::vector<RLWECt> CheetahMulImpl::RecvEncryptedArray(
    FieldType field, int64_t numel, yacl::link::Context *conn) {
  SPU_ENFORCE(impl_ != nullptr);
  return impl_->RecvEncryptedArray(field, numel, conn);
}

NdArrayRef CheetahMulImpl::RecvMulArray(FieldType field, int64_t numel,
                                        yacl::link::Context *conn) {
  SPU_ENFORCE(impl_ != nullptr);
  return impl_->RecvMulArray(field, numel, conn);
}

std::vector<RLWECt> CheetahMulImpl::EncryptArray(const NdArrayRef &inp) {
  SPU_ENFORCE(impl_ != nullptr);
  return impl_->EncryptArray(inp);
}

NdArrayRef CheetahMulImpl::MultiplyThenMask(const NdArrayRef &inp,
                                            absl::Span<RLWECt> recv_ct) {
  SPU_ENFORCE(impl_ != nullptr);
  return impl_->MultiplyThenMask(inp, recv_ct);
}

void CheetahMulImpl::Initialize(FieldType field) {
  SPU_ENFORCE(impl_ != nullptr);
