...
#include "libspu/mpc/bumblebee/ot/yacl_ferret/yacl_ote_adaptor.h"
...
uint128_t YaclFerretOTeAdaptor::yacl_id_ = 0;

void YaclFerretOTeAdaptor::OneTimeSetup() {
  if (is_setup_) {
    return;
  }
...
void YaclFerretOTeAdaptor::rcot(absl::Span<uint128_t> data) {
  if (is_setup_ == false) {
    OneTimeSetup();
  }
...
void YaclFerretOTeAdaptor::send_cot(absl::Span<uint128_t> data) {
...
void YaclFerretOTeAdaptor::recv_cot(
...
void YaclFerretOTeAdaptor::Bootstrap() {
...
void YaclFerretOTeAdaptor::BootstrapInplace(absl::Span<uint128_t> ot,
                                            absl::Span<uint128_t> data) {
  SPU_ENFORCE(ot.size() == reserve_num_);
  SPU_ENFORCE(data.size() == lpn_param_.n);
...
uint128_t YaclIknpOTeAdaptor::yacl_id_ = 0;

void YaclIknpOTeAdaptor::OneTimeSetup() {
  if (is_setup_) {
    return;
  }
