...
#include "libspu/mpc/bumblebee/arithmetic.h"
...
#include "libspu/mpc/bumblebee/nonlinear/equal_prot.h"
...
NdArrayRef TruncA::proc(KernelEvalContext* ctx, const NdArrayRef& x,
  ...
  TiledDispatch(ctx, nworker, [&](int64_t job) {
    ...
    TruncateProtocol prot(ctx->getState<BumblebeeOTState>()->get(job));
    auto out_slice =
...
NdArrayRef MsbA2B::proc(KernelEvalContext* ctx, const NdArrayRef& x) const {
  ...
  return DISPATCH_ALL_FIELDS(field, "_", [&]() {
    ...
    TiledDispatch(ctx, nworker, [&](int64_t job) {
      ...
      CompareProtocol prot(ctx->getState<BumblebeeOTState>()->get(job));
...
NdArrayRef LessAP::proc(KernelEvalContext* ctx, const NdArrayRef& x,
  ...
  bool allow_approx_less_than =
      TestEnvFlag(EnvFlag::SPU_BB_ENABLE_APPROX_LESS_THAN);
...
NdArrayRef EqualAP::proc(KernelEvalContext* ctx, const NdArrayRef& x,
  ...
  SPU_TRACE_ACTION(GET_TRACER(ctx), ctx->lctx(), (TR_MPC | TR_LAR), (~TR_MPC),
                   "equal_ap");
  int bits = TestEnvInt(EnvFlag::SPU_BB_SET_IEQUAL_BITS);
  const auto field = ctx->getState<Z2kState>()->getDefaultField();
...
NdArrayRef EqualAA::proc(KernelEvalContext* ctx, const NdArrayRef& x,
  ...
  TiledDispatch(ctx, nworker, [&](int64_t job) {
    ...
    EqualProtocol prot(ctx->getState<BumblebeeOTState>()->get(job));
    auto out_slice =
...
NdArrayRef MulA1B::proc(KernelEvalContext* ctx, const NdArrayRef& ashr,
  ...
  TiledDispatch(ctx, nworker, [&](int64_t job) {
    ...
    auto out_slice = ctx->getState<BumblebeeOTState>()->get(job)->Multiplexer(
...
NdArrayRef MulAA::squareDirectly(KernelEvalContext* ctx,
  ...
  NdArrayRef mul1;
  if (nhalf < numel) {
    auto dupx = ctx->getState<BumblebeeMulState>()->duplx();
...
NdArrayRef MulAA::proc(KernelEvalContext* ctx, const NdArrayRef& x,
  ...
  int64_t batch_sze = ctx->getState<BumblebeeMulState>()->get()->OLEBatchSize();
...
NdArrayRef MulAA::mulWithBeaver(KernelEvalContext* ctx, const NdArrayRef& x,
  ...
  const auto field = ctx->getState<Z2kState>()->getDefaultField();
  auto [a, b, c] =
      ctx->getState<BumblebeeMulState>()->TakeCachedBeaver(field, numel);
...
NdArrayRef MatMulAA::proc(KernelEvalContext* ctx, const NdArrayRef& x,
  ...
  auto* comm = ctx->getState<Communicator>();
  auto* dot_prot = ctx->getState<BumblebeeDotState>()->get();
  const int rank = comm->getRank();
...
NdArrayRef MatMulVVS::proc(KernelEvalContext* ctx, const NdArrayRef& x,
                           const NdArrayRef& y) const {
  auto out_type = makeType<bumblebee::AShrTy>(ctx->sctx()->getField());
  if (0 == x.numel() || 0 == y.numel()) {
