...
TEST_P(CompareProtTest, SpecifiedBitwidth) {
  size_t kWorldSize = 2;
  FieldType field = std::get<0>(GetParam());
  size_t radix = std::get<2>(GetParam());
  bool greater_than = std::get<1>(GetParam());
  int64_t bw = SizeOf(field) * 8 - 16;
...
TEST_P(CompareProtTest, Batch) {
  size_t kWorldSize = 2;
  int64_t n = 524288;
  int64_t batch_size = 3;

  FieldType field = std::get<0>(GetParam());
  size_t bw = SizeOf(field) * 8 - 16;
  size_t radix = std::get<2>(GetParam());
  bool greater_than = std::get<1>(GetParam());

  NdArrayRef inp[2];
  inp[0] = ring_rand(field, {n * batch_size});
  inp[1] = ring_rand(field, {n});

  DISPATCH_ALL_FIELDS(field, "set ", [&]() {
    ring2k_t mask = -1;
    if (bw < SizeOf(field) * 8) {
      mask = (static_cast<ring2k_t>(1) << bw) - 1;
    }
    NdArrayView<ring2k_t> inp0(inp[0]);
    NdArrayView<ring2k_t> inp1(inp[1]);
    inp0[0] = 1;
    inp0[1] = 10;
    inp0[2] = 100;
    ...
    pforeach(0, inp0.numel(), [&](int64_t i) { inp0[i] &= mask; });
    pforeach(0, inp1.numel(), [&](int64_t i) { inp1[i] &= mask; });
  });
  ...
  utils::simulate(kWorldSize, [&](std::shared_ptr<yacl::link::Context> ctx) {
    ...
    auto base = std::make_shared<BasicOTProtocols>(conn);
    CompareProtocol comp_prot(base, radix);
    auto b0 = ctx->GetStats()->sent_bytes.load();
    auto s0 = ctx->GetStats()->sent_actions.load();

    // CMP(x0, y0), CMP(x0, y1), ..., CMP(x0, yB)
    auto _c =
        comp_prot.BatchCompute(inp[rank], greater_than, n, bw, batch_size);

    auto b1 = ctx->GetStats()->sent_bytes.load();
    auto s1 = ctx->GetStats()->sent_actions.load();

    if (n > 8192) {
      SPDLOG_INFO(
          "Compare {} bits {} elements sent {} bytes, {} bits each #sent {}",
          bw, n, (b1 - b0), (b1 - b0) * 8. / n, (s1 - s0));
    }
    cmp_oup[rank] = _c;
  });

  auto got = ring_xor(cmp_oup[0], cmp_oup[1]);
  ...
  DISPATCH_ALL_FIELDS(field, "set ", [&]() {
    ...
    for (int64_t i = 0; i < n; ++i) {
      for (int64_t b = 0; b < batch_size; ++b) {
        bool _exp = greater_than ? (inp0[i * batch_size + b] > inp1[i])
                                 : (inp0[i * batch_size + b] < inp1[i]);
        bool _got = G[i * batch_size + b];
        ASSERT_TRUE(_exp == _got);
      }
    }
  });
}
