...
#include "libspu/mpc/bumblebee/kernels/compare.h"
...
NdArrayRef EqualAP::proc(KernelEvalContext* ctx, const NdArrayRef& x,
  ...
  SPU_TRACE_ACTION(GET_TRACER(ctx), ctx->lctx(), (TR_MPC | TR_LAR), (~TR_MPC),
                   "equal_ap");
  int iequal_bits = TestEnvInt(EnvFlag::SPU_BB_SET_IEQUAL_BITS);
  const auto field = ctx->getState<Z2kState>()->getDefaultField();
  iequal_bits = std::max(0, std::min<int>(iequal_bits, SizeOf(field) * 8));
