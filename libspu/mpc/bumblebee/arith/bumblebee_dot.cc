...
#include "libspu/mpc/bumblebee/arith/matmat_prot.h"
#include "libspu/mpc/bumblebee/rlwe/lwe_ct.h"
#include "libspu/mpc/bumblebee/rlwe/modswitch_helper.h"
#include "libspu/mpc/bumblebee/rlwe/packlwes.h"
#include "libspu/mpc/bumblebee/rlwe/utils.h"
#include "libspu/mpc/utils/ring_ops.h"

namespace spu::mpc::bumblebee {

struct BumblebeeDot::Impl : public EnableCPRNG {
 public:
  ...
  NdArrayRef DotOLE(const NdArrayRef &prv_mat, yacl::link::Context *conn,
                    ...
                    BumblebeeDot::Role role = BumblebeeDot::Role::dynamic);
...
void BumblebeeDot::Impl::LazyInitGaloisKey(size_t field_bitlen) {
...
void BumblebeeDot::Impl::LazyInit(size_t field_bitlen, bool need_galois_keys) {
  ...
  SPDLOG_INFO(
      "BumblebeeDot uses {}@{} modulus {} degree for {} bit ring. Galois "
      "generated = {}",
      ecd_modulus_sze, dcd_modulus_sze, parms.poly_modulus_degree(),
      field_bitlen, need_galois_keys);
}

void BumblebeeDot::Impl::doDotOLEReceiverRecvStep(const NdArrayRef &prv_mat,
                                                  const Shape3D &dim3,
                                                  bool is_self_lhs,
                                                  CipherPackingType cptype,
                                                  absl::Span<RLWECt> result_cts,
...
NdArrayRef BumblebeeDot::Impl::doDotOLEReceiverSendStep(
...
void BumblebeeDot::Impl::doDotOLESenderSendStep(const NdArrayRef &prv_mat,
                                                const Shape3D &dim3,
                                                bool is_self_lhs,
                                                CipherPackingType cptype,
                                                yacl::link::Context *conn) {
...
NdArrayRef BumblebeeDot::Impl::doDotOLESenderRecvStep(
    FieldType field, size_t batch_size, MatMatProtocol::Meta meta,
...
NdArrayRef BumblebeeDot::Impl::DotOLE(const NdArrayRef &prv_mat,
                                      yacl::link::Context *conn,
                                      const Shape3D &dim3, bool is_self_lhs,
                                      BumblebeeDot::Role role) {
  if (conn == nullptr) {
...
NdArrayRef BumblebeeDot::Impl::BatchDotOLE(const NdArrayRef &prv_mat,
                                           yacl::link::Context *conn,
                                           const Shape4D &dim4,
                                           bool is_self_lhs) {
  if (conn == nullptr) {
...
NdArrayRef BumblebeeDot::Impl::doBatchDotOLE(const NdArrayRef &prv_mat,
                                             yacl::link::Context *conn,
                                             const Shape4D &dim4,
                                             bool is_self_lhs) {
  auto eltype = prv_mat.eltype();
...
NdArrayRef BumblebeeDot::Impl::doDotOLE(const NdArrayRef &prv_mat,
                                        yacl::link::Context *conn,
                                        const Shape3D &dim3, bool is_self_lhs,
                                        BumblebeeDot::Role role) {
  auto eltype = prv_mat.eltype();
...
BumblebeeDot::BumblebeeDot(const std::shared_ptr<yacl::link::Context> &lctx,
                           bool enable_matmul_pack) {
  impl_ = std::make_unique<Impl>(lctx, enable_matmul_pack);
}

BumblebeeDot::~BumblebeeDot() = default;

NdArrayRef BumblebeeDot::DotOLE(const NdArrayRef &inp,
...
NdArrayRef BumblebeeDot::DotOLE(const NdArrayRef &inp, const Shape3D &dim3,
                                bool is_self_lhs, Role role) {
  SPU_ENFORCE(impl_ != nullptr);
  return impl_->DotOLE(inp, nullptr, dim3, is_self_lhs, role);
}

NdArrayRef BumblebeeDot::BatchDotOLE(const NdArrayRef &inp,
                                     yacl::link::Context *conn,
                                     const Shape4D &dim4, bool is_self_lhs) {
  SPU_ENFORCE(impl_ != nullptr);
