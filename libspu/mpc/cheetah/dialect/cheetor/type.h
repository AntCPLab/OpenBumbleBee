...
// We track the working bit width of the AShr in CHEETOR.
class AShrTy : public TypeImpl<AShrTy, RingTy, Secret, AShare> {
 ...
 public:
  ...
  explicit AShrTy(FieldType field, size_t nbits = kDefaultNumBits) {
    field_ = field;
    nbits_ = nbits == kDefaultNumBits ? SizeOf(field) * 8 : nbits;
    SPU_ENFORCE(nbits_ <= SizeOf(field) * 8);
  }

  // cast to cheetah.AShrTy
  explicit operator spu::mpc::cheetah::AShrTy() {
    return spu::mpc::cheetah::AShrTy(field_);
  }

  explicit operator PrimeShrTy();

  static std::string_view getStaticId() { return "cheetor.AShr"; }
...
class PrimeShrTy : public TypeImpl<PrimeShrTy, RingTy, Secret, AShare> {
  using Base = TypeImpl<PrimeShrTy, RingTy, Secret, AShare>;
  static constexpr size_t kDefaultNumBits = std::numeric_limits<size_t>::max();
  size_t nbits_;
 ...
 public:
  ...
  explicit PrimeShrTy(FieldType field, size_t nbits = kDefaultNumBits) {
    field_ = field;
    nbits_ = nbits == kDefaultNumBits ? SizeOf(field) * 8 : nbits;
    SPU_ENFORCE(nbits_ <= SizeOf(field) * 8);
  }

  explicit operator AShrTy();

  static std::string_view getStaticId() { return "cheetor.PrimeShr"; }
...
class BShrTy : public TypeImpl<BShrTy, RingTy, Secret, BShare> {
 ...
 public:
  ...
  // cast to cheetah.BShrTy
  explicit operator spu::mpc::cheetah::BShrTy() {
    return spu::mpc::cheetah::BShrTy(field_, nbits_);
  }

  static std::string_view getStaticId() { return "cheetor.BShr"; }
