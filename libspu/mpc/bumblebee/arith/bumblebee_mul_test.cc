...
TEST_P(BumblebeeMulTest, Basic) {
  ...
  utils::simulate(kWorldSize, [&](std::shared_ptr<yacl::link::Context> lctx) {
    ...
    auto mul = std::make_shared<BumblebeeMul>(lctx);
...
TEST_P(BumblebeeMulTest, BasicBinary) {
  size_t kWorldSize = 2;
  auto field = std::get<0>(GetParam());
  int64_t n = std::get<1>(GetParam());
...
TEST_P(BumblebeeMulTest, MixedRingSizeMul) {
  size_t kWorldSize = 2;
  auto field = std::get<0>(GetParam());
  int64_t n = std::get<1>(GetParam());
