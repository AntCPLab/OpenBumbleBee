...
size_t InitOTState(KernelEvalContext* ctx, size_t njobs) {
  ...
  size_t nworker = std::min(BumblebeeOTState::kMaxNumOtInstances,
                            CeilDiv(njobs, kMinWorkSize));
...
void BumblebeeMulState::makeSureCacheSize(FieldType field, int64_t numel) {
...
std::array<NdArrayRef, 3> BumblebeeMulState::TakeCachedBeaver(FieldType field,
                                                              int64_t numel) {
  SPU_ENFORCE(numel > 0);
